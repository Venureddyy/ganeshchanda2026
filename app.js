  ...
  expenseList.innerHTML = docs.map((d) => `
    <li>
      <span class="entry-name">
        ...
        ${d.spentBy ? `<span class="entry-subtext">by ${escapeHtml(d.spentBy)}</span>` : ""}
      </span>
      <span class="entry-meta">
        <span class="entry-amount">${fmt(d.amount)}</span>
...
expensesRef.orderBy("createdAt", "desc").onSnapshot((snap) => {
  ...
  snap.forEach((doc) => {
    const data = doc.data();
    docs.push({ id: doc.id, description: data.description, spentBy: data.spentBy, amount: data.amount });
    total += Number(data.amount || 0);
  });
  expenseTotal = total;
  renderExpenses(docs);
...
expenseForm.addEventListener("submit", (e) => {
  ...
  const spentBy = el("expenseBy").value.trim();
  const amount = Number(el("expenseAmount").value);
  if (!description || !spentBy || !amount || amount <= 0) return;
  expensesRef.add({
    description,
    spentBy,
    amount,
